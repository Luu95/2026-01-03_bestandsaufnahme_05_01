/// Vollbild-Grundriss: PDF-Ansicht, Marker setzen/verschieben und PDF-Export.
///
/// Disziplinen werden vor den Anlagen geladen, damit Marker nicht wegen
/// Race-Conditions „verschwinden“.
...
import 'package:bestandsaufnahme_01/features/floor_plans/widgets/marker_form_dialog.dart';
...
/// Vollbild-Viewer für einen [FloorPlan] inkl. Anlagen-Marker.
class FloorPlanFullScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<FloorPlanFullScreen> createState() => _FloorPlanFullScreenState();
}

/// State: PDF-Rendering, Marker-Drag und Export.
class _FloorPlanFullScreenState extends ConsumerState<FloorPlanFullScreen> {
  File? _pdfFile;
  ...
  Future<void> _loadCsvSettings() async {
    ...
    final settings = ref.read(csvSettingsProvider(projectId));
    if (!mounted) return;
