/// CSV-Import und -Export von Anlagen inkl. Foto-/ZIP-Export und Vorlagen-Header.
/// Nutzt [CsvSettings] für Spaltenmapping, Delimiter und Hierarchie-Ebenen.
...
/// Indizes einer Attribut-Dreiergruppe (Name/Type/Options/Wert) im Export-Header.
...
/// Zentraler Service für Anlagen-CSV-Import/-Export und zugehörige Datei-I/O.
class CsvService {
  ...
  static void _ensureHeaderRowFitsColumns(
    List<String> headers,
    List<_AttTripletSlotIndices> slots,
    CsvSettings csvSettings,
  ) {
    var maxIdx = 0;
  ...
  static List<_AttTripletSlotIndices> _slotsFromTripletSettings(
    List<AttributeTripletColumn> triplets,
  ) {
    return triplets
        .map(
          (q) => _AttTripletSlotIndices(
  ...
  static _TemplateStyleExportLayout _layoutFromConfiguredTriplets({
    required CsvSettings csvSettings,
    required int minAttSlots,
  }) {
    var slots = _slotsFromTripletSettings(csvSettings.attributeTripletColumns);
  ...
  static List<_AttTripletSlotIndices> _attSlotsFromSettings(
    CsvSettings csvSettings,
  ) {
    return _slotsFromTripletSettings(csvSettings.attributeTripletColumns);
  }
  ...
  static _TemplateStyleExportLayout _layoutFromHeaders(
    ...
    if (slots.length < minAttSlots && slots.isNotEmpty) {
      slots = List<_AttTripletSlotIndices>.from(slots);
    }
  ...
  static Future<_TemplateStyleExportLayout> _resolveTemplateStyleExportLayout({
    ...
    if (csvSettings.attributeTripletColumns.isNotEmpty) {
      return _layoutFromConfiguredTriplets(
        csvSettings: csvSettings,
  ...
  static Map<String, dynamic> _parseParamsFromRow(List<dynamic> row, Map<int, String> paramColumns) {
    ...
    paramColumns.forEach((idx, key) {
      final raw = CsvUtils.safeCellTrimmed(row, idx);
  ...
  /// Dünner Wrapper: Schema-Felder aus Triplet-Zeile (gemeinsamer Helfer).
  static List<Map<String, dynamic>> _schemaFieldsFromAnlagenTripletRow({
    ...
    required List<AttributeTripletColumn> triplets,
      ...
      schemaFieldsFromAnlagenTripletRow(
        row: row,
        headerRow: headerRow,
        triplets: triplets,
      );
  ...
  static void _applyAnlagenAttributeParamsFromRow({
    ...
    for (var i = 0; i < triplets.length; i++) {
      ...
      final nameCol = triplet.nameColumn;
      ...
      final nameCell = CsvUtils.safeCellTrimmed(row, nameCol);
      var paramKey = '';
      ...
      if (triplet.valueColumn < 0) continue;
      if (triplet.valueColumn == triplet.typeColumn) continue;
      if (triplet.valueColumn < headerRow.length &&
          CsvSettings.isGewerkeTypeDefinitionHeader(
            headerRow[triplet.valueColumn],
          )) {
        continue;
      }
      final cellValue = CsvUtils.safeCellTrimmed(row, triplet.valueColumn);
  ...
  static void _writeAttributeSlotFromRow({
    ...
    if (valueColumn >= 0 &&
        ...
        CsvSettings.isGewerkeTypeDefinitionHeader(headerRow[valueColumn])) {
      return;
    }
  ...
  static List<Map<String, dynamic>> _schemaFieldsFromAnlagenPairRow({
    ...
    required List<AttributeColumnPair> pairs,
      ...
      schemaFieldsFromAnlagenPairRow(row: row, pairs: pairs);
  ...
  static Set<int> _reservedImportIndices({
    ...
    required List<AttributeTripletColumn> attributeTriplets,
  }) {
    final indices = <int>{
    ...
    for (final group in attributeTriplets) {
  ...
  static Future<CsvImportResult> importAnlagenCsvForDisciplines({
    ...
    try {
      ...
      final reservedIndices = _reservedImportIndices(
        ...
        attributeTriplets: attributeTriplets,
      );
      ...
      if (useDisciplineGrouping) {
        ...
        for (var i = 0; i < dataRows.length; i++) {
          ...
          final disciplineLabel = CsvUtils.safeCellTrimmed(row, disciplineIdx);
      ...
      for (var i = 0; i < dataRows.length; i++) {
        ...
        for (var li = 0; li < enabledLevels.length; li++) {
          ...
          final levelName = CsvUtils.safeCellTrimmed(row, level.nameColumn).trim();
        ...
        final leafId = leafConfig.useIdColumn && leafConfig.idColumn != null
            ? CsvUtils.safeCellTrimmed(row, leafConfig.idColumn!).trim()
            : '';
        ...
        final disciplineLabel = useDisciplineGrouping
            ? CsvUtils.safeCellTrimmed(row, settings.level1.nameColumn)
        ...
        if (qrLabel.isNotEmpty) {
          ...
          if (qrIdx >= 0) {
            final qrVal = CsvUtils.safeCellTrimmed(row, qrIdx);
            if (qrVal.isNotEmpty) {
        ...
        } else if (attributeTriplets.isNotEmpty) {
          _applyAnlagenAttributeParamsFromRow(
            params: params,
            row: row,
            headerRow: headerRow,
            triplets: attributeTriplets,
            discipline: discipline,
            settings: settings,
          );
        }
        ...
        for (var li = 0; li < enabledLevels.length; li++) {
          ...
          final levelId = level.useIdColumn && level.idColumn != null
              ? CsvUtils.safeCellTrimmed(row, level.idColumn!).trim()
              : '';
          final dedupeToken =
