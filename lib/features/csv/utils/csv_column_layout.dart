/// Hilfsfunktionen für CSV-Spaltenlayout und Anlagen-Exportzeilen.
/// Baut Header/Zeilen 1:1 zum Import-Layout bzw. aus Spalten-Mapping.
...
void captureCsvRowCellsToParams({
  ...
  for (var i = 0; i < headerRow.length; i++) {
    ...
    cells[header] = CsvUtils.safeCellTrimmed(row, i);
  }
  params[CsvSettings.csvRowCellsParamKey] = cells;
  params[CsvSettings.csvRowIndexParamKey] = rowIndex;
}

/// Ob gespeicherte Import-Zellen für den Export vorhanden sind.
...
/// Liest die gespeicherten Header→Zellen-Mappings aus [params].
...
/// Originalzeilen-Index aus dem Import (für Sortierung beim Export).
...
/// Comparator nach gespeichertem CSV-Zeilenindex.
...
/// Export-Zeile mit exaktem Import-Spaltenlayout (1:1-Roundtrip).
/// Geänderte Werte kommen aus [params]; Metadaten aus gespeicherten Import-Zellen.
List<String> buildAnlageExportRowFromImportStructure({
  ...
  final headers = importHeaders;
  final exportDiscipline = discipline ?? anlage.discipline;
  final row = List<String>.filled(headers.length, '');
  final params = anlage.params;
  final pairs = _effectiveAttributePairs(csvSettings, importHeaders);
  // Wenn keine WERT-Paare: Tripletts für Wert-Overlay (ART-Spalten).
  final triplets = pairs.isEmpty
  ...
  int exportColForImportIndex(int importIndex) => importIndex;
  ...
  for (var ti = 0; ti < triplets.length; ti++) {
    ...
    if (triplet.valueColumn >= 0 &&
      ...
      if (value.isNotEmpty) {
        ...
        // ART-Spalte: Gruppentitel aus Schema wieder in ART schreiben.
  ...
  // Metadaten-Spalten (TYPE/OPTIONS) aus Import-Zellen auffüllen,
...
({String gewerk, String schemaItem, String bezeichnung}) readTemplateHierarchyFromRow(
  ...
  final gewerk = CsvUtils.safeCellTrimmed(row, gewerkIdx);
  final schemaItem = CsvUtils.safeCellTrimmed(row, schemaIdx);
  final bezeichnung = bezIdx == schemaIdx
      ? schemaItem
      : CsvUtils.safeCellTrimmed(row, bezIdx);
