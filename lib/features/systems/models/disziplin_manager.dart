/// UI zur gebäudespezifischen Verwaltung von Disziplinen (Gewerken) und Schemas.
/// Enthält Listenansicht, Schema-Editor und Dialog zum Anlegen/Bearbeiten.
...
class DisziplinManagerWidget extends ConsumerStatefulWidget {
  /// Gebäude, für das die Disziplinen geladen und persistiert werden.
...
class _DisziplinManagerWidgetState extends ConsumerState<DisziplinManagerWidget> {
  ...
  /// Lädt die Disziplinen des Gebäudes aus der Datenbank.
  ...
  /// Persistiert die aktuelle Disziplin-Liste für dieses Gebäude.
  Future<void> _saveDisziplinen() async {
    final dbService = ref.read(databaseServiceProvider);
    await dbService.replaceDisciplines(widget.buildingId, disziplinen);
  }

  /// Öffnet den Schema-Editor; bei mehreren Disziplinen zuerst Auswahl-Dialog.
  ...
  /// Soft-löscht zugehörige Anlagen und entfernt die Disziplin (Hard-Delete).
...
/// Dialog zum Anlegen oder Bearbeiten von Name/Darstellung einer Disziplin.
class DisziplinEditDialog extends StatefulWidget {
  /// Bestehende Disziplin; `null` = Neuaufnahme.
  final Disziplin? disziplin;
  /// Projekt-ID zum Laden eines optionalen globalen Schemas.
